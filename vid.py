...
# Initialize the Groq API key and Whisper model
GROQ_API_KEY = os.getenv('GROQ_API_KEY')
whisper_model = whisper.load_model("whisper-large-v3-turbo")

# Streamlit page configuration
st.set_page_config(
    page_title="Groq Chat Assistant with Whisper",
    page_icon="🤖",
    layout="wide",
...
def extract_audio_from_video(uploaded_video):
    ...
    audio_path = video_path.replace('.mp4', '.wav')  # Convert video to audio file
    video_clip.audio.write_audiofile(audio_path)
    
    # Use Whisper to transcribe the extracted audio
    result = whisper_model.transcribe(audio_path)
    st.success(f"Transcription from video: {result['text']}")
    return result['text']
...
def main():
    ...
    # Video file upload for transcription
    uploaded_video = st.file_uploader("Upload a video file (MP4, AVI, etc.)", type=["mp4", "avi", "mov"])
    if uploaded_video is not None:
        transcribed_text = extract_audio_from_video(uploaded_video)
        st.session_state.chat_history.append({"human": "Video input", "AI": transcribed_text})
